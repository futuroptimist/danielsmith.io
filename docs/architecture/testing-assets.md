...
| Smoke build check        | Vite production build output | `npm run smoke`                     | Ensures `npm run build` succeeds, validates `dist/index.html`, checks generated `dist/assets/*` references, and reports first-party runtime static paths. |
...
> Manual binary assets (for example `favicon.ico` and résumé PDFs) are not generated by Codex.
> Use `REQUIRE_MANUAL_STATIC_ASSETS=1 npm run smoke` after Daniel adds them to enforce strict
> source + dist checks.
