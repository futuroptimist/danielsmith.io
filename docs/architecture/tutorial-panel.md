...
- Uses progress chips/checkmarks for active tasks. Progress and completion groups render as
  block-level flow content with vertical spacing from the body copy above and navigation below
  so long localized labels can wrap without colliding with adjacent controls.
