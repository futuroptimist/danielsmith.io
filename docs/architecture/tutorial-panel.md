...
- Complete chips visibly show the localized label plus a checkmark; localized
  completed/incomplete state remains available through accessibility labels so the UI
...
- `In` and `Out` chips, localized, with compact checkmarks when complete and
  localized completed/incomplete accessibility labels.
...
- At `3/3`, the counter turns bright yellow and visibly shows the counter label plus
  a checkmark; localized completed/incomplete state remains available to assistive
  technology. After completion, the rendered counter remains `3/3` even if the shared
  POI snapshot is later reset or partially restored.
