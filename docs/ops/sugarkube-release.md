...
The chart renders `/runtime/build-info.json` into a ConfigMap, seeds it into the shared
runtime volume during pod initialization, and mounts that runtime directory read-only into nginx for
every Sugarkube deployment. The browser reads this JSON to show the Settings & Help footer release
...
same shared runtime `emptyDir`, and nginx serves that file at `githubMetricsCache.publicPath` (default
