...
- The automated test suite compiles the latest dated résumé with [Tectonic](https://tectonic-typesetting.github.io/en-US/) and [Pandoc](https://pandoc.org/) to ensure the US-Letter PDF and DOCX outputs fit on a single page. The check downloads pinned Linux binaries if they are not already on your `PATH` (other platforms should install the tools manually).
...
- ATS smoke checks require populated PDF title, author, subject, and keywords metadata; the source
  also identifies the document language as English. A word split by a line-ending hyphen is a
  blocking extraction error rather than a warning.
