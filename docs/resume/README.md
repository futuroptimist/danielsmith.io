...
- The automated test suite compiles the latest dated résumé with [Tectonic](https://tectonic-typesetting.github.io/en-US/) and [Pandoc](https://pandoc.org/) to ensure both the PDF and DOCX outputs fit on a single US Letter page. The check downloads pinned Linux binaries if they are not already on your `PATH` (other platforms should install the tools manually).
...
- The ATS smoke check treats words split by an extracted line-ending hyphen as a failure and
  validates PDF title, author, subject, keywords, and US Letter dimensions from `pdfinfo` output.
