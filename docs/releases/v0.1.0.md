...
text fallback, immersive mode, and resume evidence because it writes structured JSON to
`test-results/promotion-smoke/evidence.json`. Keep the immutable `main-<shortsha>` image tag with
that evidence so staging and production can be tied to the same artifact. All new items start
unchecked.
...
- [ ] Staging promotion smoke passed and `test-results/promotion-smoke/evidence.json` was
      attached to the release record with the immutable image tag:

  ```bash
  npm run smoke:promotion -- --base-url=https://staging.danielsmith.io
  ```

- [ ] Staging root curl passed if manual triage evidence is needed:
...
- [ ] Production promotion smoke passed and `test-results/promotion-smoke/evidence.json` was
      attached to the release record with the same immutable image tag promoted from staging:

  ```bash
  npm run smoke:promotion -- --base-url=https://danielsmith.io
  ```

- [ ] Production root curl passed if manual triage evidence is needed:
