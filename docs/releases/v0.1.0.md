...
- [ ] Capture staging promotion smoke evidence with
      `PROMOTION_SMOKE_BASE_URL=https://staging.danielsmith.io npm run smoke:promotion` before
      production promotion. Store the resulting `test-results/promotion-smoke/*.json` path next to
      the immutable image tag evidence.
...
- [ ] Prefer the promotion smoke evidence command for staging and production by changing only
      `PROMOTION_SMOKE_BASE_URL`; use the individual probes below only as manual triage.
