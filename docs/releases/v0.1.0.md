...
See the [static-site observability contract](../ops/observability.md) for the monitoring model,
paging-critical paths, release-only checks, and promotion smoke commands. This link documents the
intended contract only; it does not claim that live monitoring has already been deployed.
