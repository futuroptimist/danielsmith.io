...
- [ ] Production `https://danielsmith.io/` serves the application built from this repository rather
      than the legacy placeholder page. Prefer preserving `npm run smoke:promotion` JSON evidence
      over ad hoc browser or curl notes.
...
- [ ] Staging promotion smoke passed and `test-results/promotion-smoke/evidence.json` was
      archived with the immutable image tag:

  ```bash
  PROMOTION_SMOKE_BASE_URL=https://staging.danielsmith.io npm run smoke:promotion
  ```

- [ ] Staging root curl passed, if manual fallback evidence is needed:

  ```bash
  curl -fsS https://staging.danielsmith.io/
  ```

- [ ] Staging liveness curl passed, if manual fallback evidence is needed:

  ```bash
  curl -fsS https://staging.danielsmith.io/livez
  ```

- [ ] Staging health curl passed, if manual fallback evidence is needed:

  ```bash
  curl -fsS https://staging.danielsmith.io/healthz
  ```

- [ ] Staging resume curl passed after `/resume.pdf` exists, if manual fallback evidence is needed:
...
- [ ] Production promotion smoke passed and `test-results/promotion-smoke/evidence.json` was
      archived with the immutable image tag:
...
- [ ] Production root curl passed, if manual fallback evidence is needed:

  ```bash
  curl -fsS https://danielsmith.io/
  ```

- [ ] Production liveness curl passed, if manual fallback evidence is needed:

  ```bash
  curl -fsS https://danielsmith.io/livez
  ```

- [ ] Production health curl passed, if manual fallback evidence is needed:
