...
function pseudoRandom(index: number): number {
  const x = Math.sin(index * 12.9898 + 0.5) * 43758.5453;
...
export function createFootstepBuffer<T extends BufferContext>(
  context: T
): ReturnType<T['createBuffer']> {
  ...
  applyLoopFades(data, Math.floor(context.sampleRate * 0.015));
  normalizeBuffer(data, 0.92);
