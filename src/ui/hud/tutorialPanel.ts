...
export interface TutorialPanelHandle {
  ...
  setShowOnStartup(showOnStartup: boolean): void;
  dispose(): void;
}
...
export function createTutorialPanel({
  ...
  onPrevious?: () => void;
  onNext?: () => void;
  onToggleShowOnStartup?: (showOnStartup: boolean) => void;
}): TutorialPanelHandle {
  ...
  const render = () => {
    ...
    const hasPrevious = TUTORIAL_PAGE_ORDER.slice(0, currentIndex).some((id) =>
      currentState.unlockedPageIds.includes(id)
    );
