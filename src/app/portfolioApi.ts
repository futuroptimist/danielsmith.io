...
import type { InitialCameraFraming } from '../scene/camera/initialFraming';
import type {
...
export function ensurePortfolioApi(target: Window = window): PortfolioApi {
  target.portfolio ??= {};
  return target.portfolio;
}

export function setPortfolioSection<Key extends keyof PortfolioApi>(
  key: Key,
  value: NonNullable<PortfolioApi[Key]>,
  target: Window = window
): NonNullable<PortfolioApi[Key]> {
  const portfolio = ensurePortfolioApi(target);
  portfolio[key] = value;
  return value;
}

export function deletePortfolioSection(
  key: keyof PortfolioApi,
  target: Window = window
): void {
  delete ensurePortfolioApi(target)[key];
}
