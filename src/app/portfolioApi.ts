import type { AvatarAccessoryId, AvatarAccessoryState } from '../scene/avatar/accessories';
...
export interface PortfolioApiNamespace {
  input?: {
    keyBindings?: {
  ...
  githubMetrics?: { getDiagnostics(): GitHubRepoStatsDiagnostics };
  audio?: {
    getState(): {
...
declare global {
  interface Window {
    portfolio?: PortfolioApiNamespace;
  }
}

export type PortfolioApiSectionName = keyof PortfolioApiNamespace;

export const ensurePortfolioNamespace = (
  target: Window = window
): PortfolioApiNamespace => {
  target.portfolio ??= {};
  return target.portfolio;
};

export const setPortfolioSection = <
  SectionName extends PortfolioApiSectionName,
>(
  sectionName: SectionName,
  section: NonNullable<PortfolioApiNamespace[SectionName]>,
  target?: Window
): NonNullable<PortfolioApiNamespace[SectionName]> => {
  const namespace = ensurePortfolioNamespace(target);
  namespace[sectionName] = section;
  return section;
};

export const removePortfolioSection = <
  SectionName extends PortfolioApiSectionName,
>(
  sectionName: SectionName,
  target?: Window
): void => {
  delete ensurePortfolioNamespace(target)[sectionName];
};

export const ensurePortfolioInputNamespace = (target?: Window) => {
  const namespace = ensurePortfolioNamespace(target);
  namespace.input ??= {};
