...
function createMediaWallScreenTexture(): CanvasTexture {
  ...
  context.fillRect(canvas.width * 0.05, canvas.height * 0.16, canvas.width * 0.9, canvas.height * 0.68);
  ...
  context.fillText('Designing resilient automation, live devlogs, and deep dives.', canvas.width * 0.08, canvas.height * 0.74);

  context.font = '48px "Inter", "Segoe UI", sans-serif';
  context.fillStyle = '#9ddcff';
  context.fillText('Latest Episode · Async Flywheel Blueprints', canvas.width * 0.08, canvas.height * 0.84);
...
function createYouTubeBadgeTexture(): CanvasTexture {
  ...
  context.beginPath();
  context.moveTo(canvas.width / 2 - playWidth / 4, canvas.height / 2 - playHeight / 2);
  context.lineTo(canvas.width / 2 - playWidth / 4, canvas.height / 2 + playHeight / 2);
...
function createLivingRoomMediaWall(bounds: Bounds2D): LivingRoomMediaWallBuild {
  ...
  const trimGeometry = new BoxGeometry(trimDepth, trimHeight, boardWidth + 0.24);
  const trim = new Mesh(trimGeometry, trimMaterial);
  trim.position.set(wallInteriorX + boardDepth / 2 - trimDepth / 2, 2.35, anchorZ);
  group.add(trim);
  ...
  const screenGeometry = new PlaneGeometry(boardWidth * 0.9, boardHeight * 0.68);
  ...
  const screenGlowGeometry = new PlaneGeometry(boardWidth * 0.95, boardHeight * 0.74);
  ...
  accentLeft.position.set(wallInteriorX + boardDepth + 0.05, accentY, anchorZ - accentOffsetZ);
  ...
  badge.position.set(wallInteriorX + boardDepth + 0.04, 3.56, anchorZ + boardWidth * 0.32);
  ...
  const consoleGeometry = new BoxGeometry(0.34, consoleHeight, shelfWidth * 0.72);
