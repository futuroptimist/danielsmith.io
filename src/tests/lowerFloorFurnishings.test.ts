...
describe('lower floor furnishings foundation', () => {
  ...
  it('adds the requested backyard patio and landscaping AABBs', () => {
    ...
    expect(
      group.getObjectByName('FurnishingPart:lawnChairSeatSlat0')
    ).toBeDefined();
  ...
  it('keeps backyard patio solids in bounds and clear of blockers', () => {
    const { colliders } = createLowerFloorFurnishings();
    const p7Ids = new Set([
      'backyard-lawn-chair-west-a',
      'backyard-lawn-chair-west-b',
    ...
    const p7Colliders = colliders.filter((collider) =>
      p7Ids.has(collider.furnishingId)
    );

    expect(p7Colliders).toHaveLength(p7Ids.size);
    p7Colliders.forEach((collider, index) => {
      expect(isContainedBy(LOWER_FLOOR_ROOM_BOUNDS.backyard, collider)).toBe(
        true
      );
      p7Colliders.slice(index + 1).forEach((other) => {
        expect(rectanglesOverlap(collider, other)).toBe(false);
      });
      colliders
        .filter((other) => !p7Ids.has(other.furnishingId))
        .forEach((other) => {
          expect(rectanglesOverlap(collider, other)).toBe(false);
        });
      LOWER_FLOOR_RESERVED_BLOCKERS.forEach((blocker) => {
