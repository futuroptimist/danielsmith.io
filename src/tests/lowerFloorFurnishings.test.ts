...
describe('upper floor furnishings foundation', () => {
  ...
  it('builds the default upstairs furnishing pass', () => {
    ...
    expect(build.group.children).toHaveLength(
      DEFAULT_UPPER_FLOOR_FURNISHINGS.length
    );
    expect(build.colliders).toHaveLength(18);
    expect(build.decorativeFootprints).toHaveLength(4);
  });

  it('adds the requested upstairs furniture IDs with expected solid AABBs', () => {
    const { colliders, decorativeFootprints, group } =
      createUpperFloorFurnishings();
    const expectedSolidBounds: Record<string, RectCollider> = {
      'upper-landing-bench': {
    ...
    const expectedDecorativeBounds: Record<string, RectCollider> = {
    ...
    expect(DEFAULT_UPPER_FLOOR_FURNISHINGS.map(({ id }) => id)).toEqual([
      ...Object.keys(expectedSolidBounds).slice(0, 3),
      'upper-landing-runner',
      ...Object.keys(expectedSolidBounds).slice(3, 8),
      'creators-studio-sofa-rug',
      ...Object.keys(expectedSolidBounds).slice(8, 13),
      'loft-library-area-rug',
      ...Object.keys(expectedSolidBounds).slice(13),
      'focus-pods-soft-rug',
    ]);
    ...
    Object.entries(expectedDecorativeBounds).forEach(([id, expected]) => {
      expect(colliders.some((collider) => collider.furnishingId === id)).toBe(
        false
      );
      expect(
        decorativeFootprints.find((footprint) => footprint.furnishingId === id)
      ).toMatchObject({ bounds: expected, allowSolidOverlap: true });
      expect(group.getObjectByName(`Furnishing:${id}`)).toBeDefined();
    });
  });

  it('keeps upstairs solids in room bounds and away from blockers', () => {
