...
describe('lower floor furnishings foundation', () => {
  ...
  it('keeps the kitchen additions within bounds and outside doorway buffers', () => {
    const { colliders } = createLowerFloorFurnishings();
    const kitchenColliders = colliders.filter(
      (collider) => collider.roomId === 'kitchen'
    );
    const doorwayBuffers = [
      { minX: -5.2, maxX: -2.8, minZ: 0, maxZ: 8 },
      { minX: -22, maxX: -14, minZ: 14.8, maxZ: 17.2 },
    ];

    kitchenColliders.forEach((collider) => {
      expect(isContainedBy(LOWER_FLOOR_ROOM_BOUNDS.kitchen, collider)).toBe(
        true
      );
      doorwayBuffers.forEach((doorwayBuffer) => {
        expect(rectanglesOverlap(collider, doorwayBuffer)).toBe(false);
      });
    });
  });
