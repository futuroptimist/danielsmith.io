...
describe('lower floor furnishings foundation', () => {
  ...
  it('adds backyard patio and landscaping with requested AABBs', () => {
    ...
    const expectedBackyardBounds: Record<string, RectCollider> = {
      ...
      'backyard-grill': { minX: 26.0, maxX: 27.4, minZ: 18.35, maxZ: 19.25 },
      'backyard-prep-cart': { minX: 23.8, maxX: 25.0, minZ: 18.4, maxZ: 19.2 },
      'backyard-rock-01': { minX: 0.85, maxX: 1.35, minZ: 28.75, maxZ: 29.25 },
      'backyard-rock-02': { minX: 3.15, maxX: 3.85, minZ: 28.25, maxZ: 28.95 },
      'backyard-rock-03': { minX: 5.6, maxX: 6.2, minZ: 29.1, maxZ: 29.7 },
      'backyard-planter-west-south': {
        minX: -30.55,
        maxX: -29.85,
        minZ: 19.95,
    ...
    expect(DEFAULT_LOWER_FLOOR_FURNISHINGS.map(({ id }) => id)).toEqual(
      expect.arrayContaining(backyardIds)
    );
    expect(
    ...
    expect(gravel).toMatchObject({
      category: 'backyard',
      roomId: 'backyard',
      bounds: { minX: 0, maxX: 7, minZ: 28.05, maxZ: 30.15 },
    ...
    backyardIds.forEach((id) => {
      expect(group.getObjectByName(`Furnishing:${id}`)).toBeDefined();
    });
    expect(
      group.getObjectByName('FurnishingPart:lawnChairSlat0')
    ).toBeDefined();
    expect(group.getObjectByName('FurnishingPart:grillGrate0')).toBeDefined();
    expect(
    ...
    expect(
      group.getObjectByName('FurnishingPart:backyardPlanterStem0')
    ).toBeDefined();
  });

  it('keeps backyard solid colliders within bounds and collision exclusions', () => {
    const { colliders } = createLowerFloorFurnishings();
