...
import {
  ...
  FLYWHEEL_GEARBOX_CENTER_Y_FROM_TANGENCY,
...
describe('flywheel energy contract', () => {
  ...
  it('keeps the gearbox envelope outside the flywheel rim envelope', () => {
    ...
    expect(FLYWHEEL_GEARBOX.centerY).toBeCloseTo(
      FLYWHEEL_GEARBOX_CENTER_Y_FROM_TANGENCY
    );
    expect(FLYWHEEL_FACE_PROJECTION_VERTICAL_CLEARANCE).toBeGreaterThanOrEqual(
      FLYWHEEL_FACE_TANGENCY_CLEARANCE
    );
    expect(FLYWHEEL_FACE_PROJECTION_VERTICAL_CLEARANCE).toBeLessThanOrEqual(
      FLYWHEEL_FACE_TANGENCY_CLEARANCE_MAX
    );
