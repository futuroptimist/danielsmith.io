...
describe('audio subtitles overlay', () => {
  ...
  const getCaptionText = (): string | null => {
    const caption = document.querySelector('.audio-subtitles__caption');
    if (!caption) {
      return null;
    }
  ...
  it('queues lower-priority captions and replays them after the active clip', () => {
    ...
    expect(getCaptionText()).toBe('Ambient baseline cushions the room.');
  ...
  it('requeues preempted captions so they resume after higher-priority clips', () => {
    ...
    handle.show({
      id: 'poi-caption',
      text: 'Priority caption cuts through with project stats.',
      source: 'poi',
      priority: 5,
      durationMs: 600,
    });
    ...
    expect(getCaptionText()).toBe(
      'Priority caption cuts through with project stats.'
    );
  ...
  it('refreshes queued captions when repeated ids update their payloads', () => {
    ...
    handle.show({
      id: 'poi-caption',
      text: 'Priority caption takes the stage.',
      source: 'poi',
      priority: 3,
      durationMs: 500,
    });
    ...
    handle.show({
      id: 'ambient-hum',
      text: 'Updated ambient copy after priority caption.',
      source: 'ambient',
      priority: 2,
      durationMs: 700,
    });

    expect(getCaptionText()).toBe('Priority caption takes the stage.');

    vi.advanceTimersByTime(500);
    expect(getCaptionText()).toBe(
      'Updated ambient copy after priority caption.'
    );
  ...
  it('clears queued ambient captions by id without dropping active priority caption', () => {
    const handle = createAudioSubtitles();
    handle.show({
      id: 'poi-caption',
      text: 'Priority caption remains active while ambient is muted.',
      source: 'poi',
      priority: 4,
      durationMs: 1000,
    });
    ...
    expect(getCaptionText()).toBe(
      'Priority caption remains active while ambient is muted.'
    );
    vi.advanceTimersByTime(1000);
    expect(document.querySelector('.audio-subtitles')?.dataset.visible).toBe(
      'false'
  ...
  it('clears the active and queued captions when clear is called without an id', () => {
    ...
    handle.show({
      id: 'poi-caption',
      text: 'Long-running priority caption clip.',
      source: 'poi',
      priority: 4,
      durationMs: 5000,
    });
  ...
  it('upgrades aria-live to assertive for high-priority captions and resets after hide', () => {
    ...
    handle.show({
      id: 'poi-critical',
      text: 'Critical priority caption demands immediate attention.',
      source: 'poi',
      priority: 6,
      durationMs: 1200,
    });
  ...
  it('honours custom assertive thresholds when provided', () => {
    ...
    handle.show({
      id: 'poi-critical',
      text: 'Critical priority caption would normally elevate priority.',
      source: 'poi',
      priority: 6,
      durationMs: 1000,
    });
  ...
  it('renders an accessible dismiss button while a message is visible', () => {
    ...
    handle.show({
      id: 'poi-caption',
      text: 'Priority caption is visible until dismissed.',
      source: 'poi',
      durationMs: 5000,
    });
    ...
    expect(button?.getAttribute('aria-label')).toBe('Dismiss priority caption');
  ...
  it('dismiss button hides the current message and clears queued captions', () => {
    ...
    handle.show({
      id: 'poi-caption',
      text: 'Priority caption is active.',
      source: 'poi',
      priority: 5,
      durationMs: 5000,
    });
