...
const baseDefinitions: PoiStaticDefinition[] = [
  ...
  },
  {
    ...
    pedestal: {
      ...
      bodyColor: 0x101a28,
      bodyOpacity: 0.54,
      renderInPerformance: true,
      performanceBodyOpacity: 0.36,
      ...
      accentOpacity: 0.92,
      performanceAccentOpacity: 0.72,
      ringColor: 0x6affdf,
      ringOpacity: 0.65,
      performanceRingOpacity: 0.45,
      orbColor: 0xbefde4,
...
function localizeBaseDefinitions(input?: LocaleInput): PoiDefinition[] {
  ...
  return baseDefinitions.map((base) => {
    ...
    return {
      ...
      environments: copy.environments?.map((environment) => ({
        ...environment,
      })),
      interactionPrompt,
...
function clonePoi(definition: PoiDefinition): PoiDefinition {
  return {
    ...
    environments: definition.environments?.map((environment) => ({
      ...environment,
    })),
