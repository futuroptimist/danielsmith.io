...
function parseStoredPayload(
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && 'accessories' in parsed) {
      const payload = parsed as { accessories?: unknown };
      return parseBooleanRecord(payload.accessories, validAccessoryIds);
    }
    return parseBooleanRecord(parsed, validAccessoryIds);
  } catch (error) {
...
function serializeState(payload: StoredState): string {
...
export function createAvatarAccessoryManager({
  ...
  if (storage?.getItem) {
    try {
      ...
      storedState = parseStoredPayload(raw, validIds);
    } catch (error) {
      console.warn(
