...
export function createAvatarAccessoryControl({
  ...
  options.forEach((option) => {
    ...
    input.type = 'checkbox';
    input.className = 'avatar-accessories__checkbox';
    input.value = option.id;

    const copy = document.createElement('span');
    copy.className = 'avatar-accessories__copy';
    ...
    copy.append(titleSpan, descriptionSpan);
    label.append(input, copy);
    list.appendChild(label);

    input.addEventListener('change', () => {
      handleToggle(option.id, input.checked);
    });
