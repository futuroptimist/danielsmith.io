...
describe('getCameraRelativeMovementVector', () => {
  ...
  it('maps backward input toward the camera', () => {
    const camera = createCamera();
    const result = getCameraRelativeMovementVector(
      camera,
      0,
      -1,
      new Vector3()
  ...
  it('combines forward and right input without losing either component', () => {
    const camera = createCamera();
    const forwardOnly = getCameraRelativeMovementVector(
      camera,
      0,
      1,
      new Vector3()
    );
    const rightOnly = getCameraRelativeMovementVector(
      camera,
      1,
      0,
      new Vector3()
    );
    const combined = getCameraRelativeMovementVector(
      camera,
      1,
      1,
      new Vector3()
