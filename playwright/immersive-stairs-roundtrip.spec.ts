  ...
  return page.evaluate(
    ({
      ...
      for (let index = 0; index < 180; index += 1) {
        ...
        if (!result.movedZ) {
          throw new Error(
            `Runtime descent step rejected at z=${before.z.toFixed(2)} by ${(result.blockedBy ?? []).join(', ')}`
          );
        }
        if (Math.abs(result.position.x - nextStairCenterX) > 0.001) {
          throw new Error(
...
async function stepRuntimeTowardBlockedUpperStairEdge(
  page: Page,
...
test('upper landing debug colliders exclude middle landing artifact', async ({
  ...
  const removedBroadStairBlockers = ['UpperStairDeepVoidBlocker'];
  ...
  const noFloorCutoutSamples: NamedPosition[] = [
    ...
    {
      name: 'unsafe upper stair edge sample',
...
test('upper landing opens west into upstairs rooms and blocks unsafe stair edges', async ({
  page,
}) => {
  test.slow();
  await waitForImmersiveReady(page);
  ...
  expect(await canOccupyPosition(page, upperStairTopRun)).toBe(true);
  expect(await getBlockingColliderNames(page, upperStairTopRun)).toEqual([]);
  for (const sample of formerLandingArtifactSamples) {
    expectSampleOnPhysicalStaircaseLanding(sample, stairMetrics);
    expect(await canOccupyPosition(page, sample.target), sample.name).toBe(
      true
  ...
  const runtimeWestEntry = await stepRuntimeTowardBlockedUpperStairEdge(
    page,
    { x: 8.1, z: -24.68 },
    { dx: 0.12, dz: 0 }
  );
